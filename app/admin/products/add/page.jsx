...
export default function AddProduct() {
    ...
    const [colors, setColors] = useState([{ code: "#4F46E5", name: "" }])
    const [enableDimRange, setEnableDimRange] = useState(false)
    const [dimRange, setDimRange] = useState({ minH: "", maxH: "", minW: "", maxW: "", minD: "", maxD: "" })
    ...
    const validate = () => {
        const errs = {}
        if (!selectedFile) errs.thumbnail = t("noUploadImgError")
        ...
        if (!price || isNaN(Number(price)) || Number(price) <= 0) errs.price = t("positiveNumber")
        const disc = Number(discount)
        if (discount === "" || isNaN(disc) || disc < 0 || disc > 100) errs.discount = "Must be 0–100"
        if (stock === "" || isNaN(Number(stock)) || Number(stock) < 0) errs.stock = t("positiveNumber")
        if (!material.trim()) errs.material = t("fieldRequired")
        if (!dimensions.trim()) errs.dimensions = t("fieldRequired")
        if (!weight || isNaN(Number(weight)) || Number(weight) <= 0) errs.weight = t("positiveNumber")
        if (!colors.some(c => c.name.trim())) errs.colors = t("atLeastOneColorRequired")
        if (enableDimRange) {
    ...
    const handleSubmit = async (e) => {
        ...
        const allTouched = Object.fromEntries(
            ["thumbnail","name","category","description","price","discount","stock",
             "material","dimensions","weight","colors",...(enableDimRange?["dimRange"]:[])].map(k => [k, true])
        )
    ...
    return (
        <div className="max-w-3xl">
            ...
            <div className="flex items-center gap-3 mb-8">
                ...
                <div>
                    <span className="block text-xl font-bold text-gray-900">{t('add_product')}</span>
                    <span className="block text-sm text-gray-500 mt-0.5">{t("product_subtitle")}</span>
                </div>
            </div>
                ...
                <FormCard title="Product Image" description="Main photo shown to customers in the shop">
                    ...
                    <div className="flex items-start gap-6">
                        <div
                            ...
                            className={`relative w-36 h-36 rounded-2xl border-2 border-dashed flex items-center justify-center
                        ...
                        >
                            {previewUrl ? (
                                ...
                                <div className="flex flex-col items-center gap-2.5 p-4 text-center">
                                    <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${err('thumbnail') ? "bg-rose-100" : "bg-gray-100"}`}>
                        ...
                        <div className="flex flex-col justify-center gap-2.5 pt-2">
                            <span className="text-sm font-medium text-gray-800">Upload product thumbnail</span>
                            ...
                            <div key={i} className="flex items-center gap-2">
                                <div className="flex-1 relative">
                                    <Input
                                        ...
                                        className="h-10 bg-gray-50 border-gray-200 text-sm focus-visible:ring-orange-200 focus:border-orange-400"
                                    />
                                </div>
                                {features.length > 1 && (
                                    <button
